...
import { getBoroughBySlug, getAllBoroughs } from '../belfast-boroughs';
import BelfastBoroughClient from './BelfastBoroughClient';
...
const cityConfig = getLocationBySlug('belfast')!;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://pestproindex.com/pest-control/belfast/${data.slug}`,
    },
  };
}

export default async function BelfastBoroughPage({ params }: Props) {
  const { borough } = await params;
  ...
  const jsonLd = {
    ...
    url: `https://pestproindex.com/pest-control/belfast/${data.slug}`,
  };
  ...
  return (
    <>
      ...
      />
      <BelfastBoroughClient borough={data} initialProviders={providers} />
    </>
  );
}
