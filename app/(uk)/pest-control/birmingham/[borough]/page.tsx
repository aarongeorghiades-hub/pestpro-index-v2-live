...
import { getBoroughBySlug, getAllBoroughs } from '../birmingham-boroughs';
import BirminghamBoroughClient from './BirminghamBoroughClient';
import { getPestBySlug, getLocationBySlug } from '@/app/(uk)/pest-control/pest-city-config';
...
const cityConfig = getLocationBySlug('birmingham')!;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://pestproindex.com/pest-control/birmingham/${data.slug}`,
    },
  };
}

export default async function BirminghamBoroughPage({ params }: Props) {
  const { borough } = await params;
  ...
  const jsonLd = {
    ...
    url: `https://pestproindex.com/pest-control/birmingham/${data.slug}`,
  };
  ...
  return (
    <>
      ...
      />
      <BirminghamBoroughClient borough={data} initialProviders={providers} />
    </>
  );
}
