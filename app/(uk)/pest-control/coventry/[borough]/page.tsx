...
import { getBoroughBySlug, getAllBoroughs } from '../coventry-boroughs';
import CoventryBoroughClient from './CoventryBoroughClient';
...
const cityConfig = getLocationBySlug('coventry')!;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://pestproindex.com/pest-control/coventry/${data.slug}`,
    },
  };
}

export default async function CoventryBoroughPage({ params }: Props) {
  const { borough } = await params;
  ...
  const jsonLd = {
    ...
    url: `https://pestproindex.com/pest-control/coventry/${data.slug}`,
  };
  ...
  return (
    <>
      ...
      />
      <CoventryBoroughClient borough={data} initialProviders={providers} />
    </>
  );
}
