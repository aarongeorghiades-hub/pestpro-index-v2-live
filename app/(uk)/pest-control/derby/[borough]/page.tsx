...
import { getBoroughBySlug, getAllBoroughs } from '../derby-boroughs';
import DerbyBoroughClient from './DerbyBoroughClient';
...
const cityConfig = getLocationBySlug('derby')!;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://pestproindex.com/pest-control/derby/${data.slug}`,
    },
  };
}

export default async function DerbyBoroughPage({ params }: Props) {
  const { borough } = await params;
  ...
  const jsonLd = {
    ...
    url: `https://pestproindex.com/pest-control/derby/${data.slug}`,
  };
  ...
  return (
    <>
      ...
      />
      <DerbyBoroughClient borough={data} initialProviders={providers} />
    </>
  );
}
