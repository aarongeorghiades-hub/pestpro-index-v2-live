...
import { getBoroughBySlug, getAllBoroughs } from '../nottingham-boroughs';
import NottinghamBoroughClient from './NottinghamBoroughClient';
...
const cityConfig = getLocationBySlug('nottingham')!;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://pestproindex.com/pest-control/nottingham/${data.slug}`,
    },
  };
}

export default async function NottinghamBoroughPage({ params }: Props) {
  const { borough } = await params;
  ...
  const jsonLd = {
    ...
    url: `https://pestproindex.com/pest-control/nottingham/${data.slug}`,
  };
  ...
  return (
    <>
      ...
      />
      <NottinghamBoroughClient borough={data} initialProviders={providers} />
    </>
  );
}
