...
export const metadata = cityDirectoryMetadata('Birmingham', 'birmingham');
...
export default async function ResidentialPage() {
  ...
  if (error) console.error('[SSR fetch] birmingham-residential:', error.message);
  const providers = (data || []).map((p: any) => ({ ...p, postcode: p.postcode || extractPostcode(p.address) }));
  return (
    <>
      <ListingSchema
        providers={providers}
        listName={`Pest Control Providers in Birmingham`}
        listUrl="/birmingham/residential"
        areaName="Birmingham"
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Birmingham', url: '/birmingham/residential' },
        ]}
      />
