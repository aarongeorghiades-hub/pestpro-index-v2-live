...
export const metadata = cityDirectoryMetadata('Bradford', 'bradford');
...
export default async function ResidentialPage() {
  ...
  if (error) console.error('[SSR fetch] bradford-residential:', error.message);
  const providers = (data || []).map((p: any) => ({ ...p, postcode: p.postcode || extractPostcode(p.address) }));
  return (
    <>
      <ListingSchema
        providers={providers}
        listName={`Pest Control Providers in Bradford`}
        listUrl="/bradford/residential"
        areaName="Bradford"
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Pest Control', url: '/pest-control' },
          { name: 'Bradford', url: '/bradford/residential' },
        ]}
      />
