...
export const metadata = cityDirectoryMetadata('Brighton', 'brighton');
...
export default async function ResidentialPage() {
  ...
  if (error) console.error('[SSR fetch] brighton-residential:', error.message);
  const providers = (data || []).map((p: any) => ({ ...p, postcode: p.postcode || extractPostcode(p.address) }));
  return (
    <>
      <ListingSchema
        providers={providers}
        listName={`Pest Control Providers in Brighton`}
        listUrl="/brighton/residential"
        areaName="Brighton"
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Pest Control', url: '/pest-control' },
          { name: 'Brighton', url: '/brighton/residential' },
        ]}
      />
