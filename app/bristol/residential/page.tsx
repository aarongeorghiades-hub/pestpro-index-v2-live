...
export const metadata = cityDirectoryMetadata('Bristol', 'bristol');
...
export default async function ResidentialPage() {
  ...
  if (error) console.error('[SSR fetch] bristol-residential:', error.message);
  const providers = (data || []).map((p: any) => ({ ...p, postcode: p.postcode || extractPostcode(p.address) }));
  return (
    <>
      <ListingSchema
        providers={providers}
        listName={`Pest Control Providers in Bristol`}
        listUrl="/bristol/residential"
        areaName="Bristol"
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Pest Control', url: '/pest-control' },
          { name: 'Bristol', url: '/bristol/residential' },
        ]}
      />
