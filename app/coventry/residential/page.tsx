...
export const metadata = cityDirectoryMetadata('Coventry', 'coventry');
...
export default async function ResidentialPage() {
  ...
  if (error) console.error('[SSR fetch] coventry-residential:', error.message);
  const providers = (data || []).map((p: any) => ({ ...p, postcode: p.postcode || extractPostcode(p.address) }));
  return (
    <>
      <ListingSchema
        providers={providers}
        listName={`Pest Control Providers in Coventry`}
        listUrl="/coventry/residential"
        areaName="Coventry"
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Coventry', url: '/coventry/residential' },
        ]}
      />
