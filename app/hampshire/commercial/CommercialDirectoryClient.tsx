...
import { Phone, ShieldCheck, Award, CheckCircle2 } from 'lucide-react';
...
function computeQualityScore(provider: Provider): number {
  let score = 0;
  Object.values(FILTER_CATEGORIES).forEach((category) => {
...
function computeFilterCounts(data: Provider[]): FilterCounts {
...
export default function CommercialDirectoryClient({ initialProviders }: { initialProviders: Provider[] }) {
  const providers = initialProviders;
  const [filteredProviders, setFilteredProviders] = useState<Provider[]>(() => {
    const sorted = [...initialProviders];
    sorted.sort((a, b) => computeQualityScore(b) - computeQualityScore(a));
    return sorted;
  });
  const [filterCounts] = useState<FilterCounts>(() => computeFilterCounts(initialProviders));
      ...
      <section className="relative h-[500px] overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-4 h-full flex flex-col justify-center items-center text-center">
          ...
          <p className="text-lg sm:text-xl md:text-3xl lg:text-4xl mb-6 font-extralight tracking-[0.15em] text-white drop-shadow-[0_8px_20px_rgba(0,0,0,0.9)]">
            {filteredProviders.length} Verified Providers in Hampshire
          </p>
      ...
      <section className="relative bg-gradient-to-br from-yellow-50 to-white py-16 border-b-2 border-yellow-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600">Highest-rated commercial pest control providers in Hampshire</p>
          </div>
      ...
      <section className="bg-gray-50 border-t border-gray-200 py-12">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <p className="text-gray-700 mb-4">
            ...
            {' | '}
            <Link href="/leeds/commercial" className="text-blue-600 hover:underline font-semibold">
              Leeds Commercial
            </Link>
            {' | '}
            ...
            {' | '}
            <Link href="/glasgow/commercial" className="text-blue-600 hover:underline font-semibold">
              Glasgow Commercial
            </Link>
            {' | '}
            <Link href="/newcastle/commercial" className="text-blue-600 hover:underline font-semibold">
              Newcastle Commercial
            </Link>
            {' | '}
            <Link href="/cardiff/commercial" className="text-blue-600 hover:underline font-semibold">
              Cardiff Commercial
            </Link>
            {' | '}
            <Link href="/edinburgh/commercial" className="text-blue-600 hover:underline font-semibold">
              Edinburgh Commercial
            </Link>
            {' | '}
            <Link href="/leicester/commercial" className="text-blue-600 hover:underline font-semibold">
              Leicester Commercial
            </Link>
            {' | '}
            <Link href="/hampshire/commercial" className="text-blue-600 hover:underline font-semibold">
