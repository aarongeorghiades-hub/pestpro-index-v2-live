...
export const metadata = cityDirectoryMetadata('Leeds', 'leeds');
...
export default async function ResidentialPage() {
  ...
  const { data, error } = await supabase
    ...
    .or('regions.cs.["leeds"]');
  if (error) console.error('[SSR fetch] leeds-residential:', error.message);
  const providers = (data || []).map((p: any) => ({ ...p, postcode: p.postcode || extractPostcode(p.address) }));
  return (
    <>
      <ListingSchema
        providers={providers}
        listName={`Pest Control Providers in Leeds`}
        listUrl="/leeds/residential"
        areaName="Leeds"
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Leeds', url: '/leeds/residential' },
        ]}
      />
