...
export const metadata = cityDirectoryMetadata('Leicester', 'leicester');
...
export default async function ResidentialPage() {
  ...
  if (error) console.error('[SSR fetch] leicester-residential:', error.message);
  const providers = (data || []).map((p: any) => ({ ...p, postcode: p.postcode || extractPostcode(p.address) }));
  return (
    <>
      <ListingSchema
        providers={providers}
        listName={`Pest Control Providers in Leicester`}
        listUrl="/leicester/residential"
        areaName="Leicester"
        breadcrumbs={[
          { name: 'Home', url: '/' },
          { name: 'Pest Control', url: '/pest-control' },
          { name: 'Leicester', url: '/leicester/residential' },
        ]}
      />
