...
import { getBoroughBySlug } from '../sheffield-boroughs';
...
const cityConfig = getLocationBySlug('sheffield')!;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!data) return {};
  return {
    title: `Pest Control ${data.name} | Sheffield & South Yorkshire`,
...
export default async function SheffieldBoroughPage({ params }: Props) {
  ...
  return (
    <>
      ...
      />
      <SheffieldBoroughClient borough={data} initialProviders={providers} />
    </>
  );
}
